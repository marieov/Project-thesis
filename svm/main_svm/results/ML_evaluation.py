...
def plot_importances(file_name, model, names, X_test, y_test, find_important_channels=False, find_important_features=False, mdi=False, mda=False):
    ...
    np.savetxt("names_" + str(model)[0] + 'feature_channel' + str(find_important_features) + str(find_important_channels), names,  fmt="%s")
    np.savetxt('importance_sorted_mda' + str(mda) + 'mdi' + str(mdi) + 'tree' + str(tree) + 'feature_channel' + str(find_important_features) + str(find_important_channels) + '.txt', np.array([importance_sorted]), fmt="%s")
...
def plot_accuracies(accuracies, importances_decending):
    ...
    plt.xticks([i for i in range(1,len(list(importances_decending.keys()))+1)], rotation =60) 
    plt.ylabel("Accuracy")
    plt.ylim([0,1])
    plt.tight_layout()
