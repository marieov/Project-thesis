...
feat_file_features = "features_f_no_nans_3feat.csv"

feat_file_channels = "features_c.txt"
labels_file_channels = "labels_c.txt"
channels_file_channels = "channel_names_c.txt"
importances_decending_file = "channels_importance.txt"

feature_names = ["std", "rms", "katz_fractal_dimension"]#var", "ptp_amp", "kurtosis", "instantaneous_energy", "teager_energy", "higuchi_fractal_dimension","petrosian_fractal_dimension", "std", "skewness", 
#"line_length", "rms", "hjort_mobility", "hjort_complexitY", "katz_fractal_dim", "sevcik_fraction_dim","mean"] # names needed for plotting

models = ["svm"]

for model in models:
    print("----------- BEGIN MODEL ", model, "-----------")
    ...
    def classification(model, features, labels, names): # TODO dårlig navn
        ...
        if find_important_channels:
            file_name = model + "_channels.eps"
        elif find_important_features:
            file_name = model + "_features.eps"
        start = time.time()
        if model == "rf": 
    ...
    np.savetxt(importances_decending_file, np.array([importances_decending]), fmt="%s")
    ...
    if not (os.path.exists("acc_" + str(model) + ".txt")): 
        ...
        for channel in importances_decending.keys(): # channel is an int # TODO: DENNE ER FRA DEN SISTE I MODELS, så det blir svm hver gang
            ...
            file_name = model + str(channel) + ".eps"
            if rf:
