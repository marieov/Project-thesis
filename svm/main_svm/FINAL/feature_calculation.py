...
def higuchi_fractal_dimension(data, k_max=None):
    if math.isnan(hfda.measure(data, 10)): 
        print("is nan")
        return 0 # TODO: dette er kanskje ikke helt optiamlt... hvis denne ikke brukes: ta vekk
...
def calculate_features(dwt, feature_vector, seizure):
    ...
    for dwt_value in dwt:
        ...
        feature += [#variance(dwt_value),
                    #ptp_amp(dwt_value), 
                    #kurtosis(dwt_value),
                    #instantaneous_energy(dwt_value), # SVM: 
                    #teager_energy(dwt_value),                     
                    #higuchi_fractal_dimension(dwt_value),                    
                    #petrosian_fractal_dimension(dwt_value),
                    std(dwt_value), 
                    #skewness(dwt_value),              
