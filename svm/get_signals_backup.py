...
def add_signal_to_channel(signal, channel, channel_signals, seizure): 
    if not all((s == 0.0 or s == signal[0]) for s in signal) and channel not in config.unwanted_channels: # channel not in us
...
def get_seizure_start_end(fname_path, file): 
    '''
        ...
        param fname_path: path to the file, string
...
def get_signal_start_interval(signal, channel, channel_signals, start_time, end_time, interval):
    seizure = False
    
    seizure_start = int(start_time[interval]) # in seconds                    
    seizure_free_signal = signal[:int(seizure_start*config.Fs)]

    channel_signals = add_signal_to_channel(seizure_free_signal, channel, channel_signals, seizure)
    return channel_signals
...
def get_signal_seizure_interval(signal, channel, channel_signals, start_time, end_time, interval):
    ...
    channel_signals = add_signal_to_channel(seizure_signal, channel, channel_signals, seizure)
    return channel_signals
...
def get_signal_last_interval(signal, channel, channel_signals, start_time, end_time, interval):
    seizure = False
...
def get_signal_from_interval(signal, channel, channel_signals, start_time, end_time, interval):
    ...
    if interval == 0: # start of the signal
        channel_signals = get_signal_start_interval(signal, channel, channel_signals, start_time, end_time, interval)
       
    elif (interval % 2) != 0: # seizure
        channel_signals = get_signal_seizure_interval(signal, channel, channel_signals, start_time, end_time, interval)
    
    else: 
        number_of_seizures = len(start_time)
        if interval == (number_of_seizures*2): #last part
            channel_signals = get_signal_last_interval(signal, channel, channel_signals, start_time, end_time, interval)

        else: #intermediate part
            channel_signals = get_signal_intermediate_interval(signal, channel, channel_signals, start_time, end_time, interval)
...
def get_signal_from_file(edf_file, channel_no):
    ...
    signal = signal[::config.downsampling].tolist()
    return signal


def get_signal_from_files(files, files_seizures, patient_number, channel_signals):
    '''
    Reads the signal for each trail and each channel.      
    If the file has an epileptic seizure the seizure is splitted into 6 sec segments.
    ...
    for file in files:
        if file.startswith('chb' + str(patient_number)): # this is added to make it easier to run for fewer patients 
            ...
            if file in files_seizures: # If the file has a seizure we need to separate the seizure signal from the non-seizure signal
                ...
                start_time, end_time = get_seizure_start_end(fname_path, file.strip())
                ...
                for interval in range(intervals):
                    ...
                    for channel_no,channel in enumerate(channel_names): 
                        ...
                        channel_signals = get_signal_from_interval(signal, channel, channel_signals, start_time, end_time, interval)
            ...
            # No seizure in file
                ...
                for channel_no,channel in enumerate(channel_names):
                    channel = channel.split("-")[0]
                    signal = get_signal_from_file(edf_file, channel_no)
                    channel_signals = add_signal_to_channel(signal, channel, channel_signals, seizure = False)

    return channel_signals

def add_data_to_dataframe(files, files_seizures, patient_number, df):
        ...
        if file.startswith('chb' + str(patient_number)): # this is added to make it easier to run for fewer patients 
            ...
            if file in files_seizures: # If the file has a seizure we need to separate the seizure signal from the non-seizure signal
                ...
                for interval in range(intervals):
                    ...
                    print('Interval number', interval+1, 'of', intervals) # debugging
                    if interval%2==1: # odd numbers are seizure intervals
                        seizure=1
                    else: 
                        seizure=0
                    ...
                    for channel_no,channel in enumerate(channel_names): 
                        channel = channel.split("-")[0]
                        signal = get_signal_from_file(edf_file, channel_no) # TODO: de 4 neste linjene er det eneste om er forskjellig fra get_signal_fromfiles fuksjonen, så disse kan nok slås mmen
                        #channel_signals = get_signal_from_interval(signal, channel, channel_signals, start_time, end_time, interval)
